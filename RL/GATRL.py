...
# grid_x, grid_y = torch.meshgrid(torch.tensor(range(2708)), torch.tensor(range(2708)), indexing='ij')
# grid_x = torch.reshape(grid_x, (-1,1))
# grid_y = torch.reshape(grid_y, (-1,1))
# edge_index2 = torch.cat((grid_x, grid_y), 1).type(torch.LongTensor).T.to(device)
...
class GAT(torch.nn.Module):
    def __init__(self):
        ...
        self.conv1 = TransformerConv(dataset.num_features, self.hid, heads=self.in_head, dropout=0.6)
        self.conv2 = TransformerConv(self.hid * self.in_head, dataset.num_classes, concat=False,
